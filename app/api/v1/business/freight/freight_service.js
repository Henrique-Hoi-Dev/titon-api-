...
class FreightService extends BaseService {
    ...
    async updateFreightDriver(body, freightId, { id, name }) {
        ...
        if (!financial.start_km && body.truck_current_km) {
            const financialInstance = await this._financialStatementModel.findByPk(financial.id);
            if (financialInstance) {
                await financialInstance.update({
                    start_km: body.truck_current_km
                });
            }
        }
